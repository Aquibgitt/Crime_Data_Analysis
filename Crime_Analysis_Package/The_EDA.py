...
class eda_Exploration:
    ...
    def crime_by_gender_2(): 
        ...
        grouped_data = df.groupby(['Age_Group', 'Vict Sex']).size().reset_index(name='Count')

        plt.figure(figsize=(15, 6))
        # Plotting using Seaborn barplot
        sns.barplot(data=grouped_data, x='Age_Group', y='Count', hue='Vict Sex', palette=['blue', 'pink', 'green', 'red', 'orange'])
    ...
    def Result_1():
        ...
        sns.countplot(data=df, x='Time Zone', hue='Crm Cd Desc', order= time_zone_order, palette='Set2', dodge=True)
